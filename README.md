...
出差报销工具现在默认先在对话框做启动检查，不会一上来就直接长时间执行。用户只需要把材料发给 Agent，Agent 第一条回复应先告诉你：

- 报销材料是否已经收到。
- 公司报销 Excel 表格是否已经收到，或者是否可以复用上次配置。
- 公司开票信息和报销规则是否缺失。
- 上传的图片、PDF、Excel 准备保存到哪个工作目录。
- 缺少文件时能继续做什么，不能继续做什么。
- 下一步是否开始保存和扫描，并等待你确认。
...
- [x] 报销材料：看到 18 个附件，准备保存到 `reimbursement-runs/20260624-153000/raw/`
- [ ] 公司报销表格：未找到，请上传 Excel；如果之前配置过，我可以复用上次模板
- [ ] 公司开票信息：未找到，稍后只问缺失字段
- [x] 输出位置：建议使用 `reimbursement-runs/20260624-153000/`

下一步：回复“开始”后，我只保存和扫描文件；回复“改路径：...”可以换保存位置。
```
...
Agent 应先在对话框询问是否使用默认工作目录。用户确认后，才把附件保存到工作目录，再按 `AGENT.md` 的流程扫描、识别、分类和确认。默认目录结构如下：
...
1. 在对话框做启动检查，告诉用户缺什么、建议保存在哪里。
2. 等用户回复“开始”或修改保存位置。
3. 保存上传文件或复制材料文件夹到本次运行目录。
4. 只做轻量扫描并汇报文件数量。
5. 再询问是否开始识别图片、PDF 和付款截图。
6. 识别、归类后，直接在对话框展示逐笔勾选确认清单。
7. 等用户在对话框确认异常项、低置信度项和是否纳入报销。
8. 确认后再按公司规则预审、填 Excel、输出完整报销包。

![逐笔确认面板](docs/images/confirmation-panel.svg)

确认清单必须直接显示在对话框里。Markdown/CSV 文件只用于留档，不应该要求用户打开文件才能确认。确认清单会采用复选框形式，方便用户直接回复要纳入、排除或修改哪一笔：
...
Agent 可按下面命令编排，不要求普通用户手动执行。默认必须分阶段执行，不能在第一条回复前跑完整命令流。
...
- Agent 第一条回复先在对话框做启动检查，明确告诉用户“看到什么、缺什么、准备保存在哪里、下一步要不要开始”。
- 长耗时识别前必须先问用户；不能让用户等几分钟后才知道发生了什么。
- Agent 自动扫描、识别、归类、列异常，但必须分步骤推进，不让用户从空表开始填。
- 每一笔费用都在对话框可见，可修改，可排除。
- 明显可报销项默认建议纳入，但仍展示来源和置信度。
- 低置信度、付款记录缺发票、税号不一致、日期不在行程内、疑似私人消费必须提示用户确认。
- 确认以对话框为主，文件只做备份和最终交付。
